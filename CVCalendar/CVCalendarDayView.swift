    ...
    public func labelSetup() {
        ...
        if isOut {
            ...
            if coordinator?.selectedDayView == nil && calendarView.shouldAutoSelectDayOnMonthChange {
                ...
                if ((appearance?.dayLabelPresentWeekdayInitallyBold!) != nil) {
    ...
    public func setupDotMarker() {
        ...
        if let delegate = calendarView.delegate {
            if let shouldShow = delegate.dotMarker?(shouldShowOnDayView: self) , shouldShow {
                ...
                let colors = isOut ? [UIColor.gray] : delegate.dotMarker?(colorOnDayView: self)
...
extension CVCalendarDayView {
    ...
    public func moveView(_ view: UIView, onCircleView circleView: UIView, fromAngle angle: CGFloat, toAngle endAngle: CGFloat, straight: Bool) {
...
extension CVCalendarDayView {
    public func setSelectedWithType(_ type: SelectionType) {
        ...
        case .single:
            ...
            if let delegate = calendarView.delegate, let shouldShowCustomSelection = delegate.shouldShowCustomSingleSelection?() , shouldShowCustomSelection {
